...
#        EE_POS_TGT = np.asmatrix([0.3305805, -0.1326121, 0.4868]) # center of the H
        EE_POS_TGT = np.asmatrix([0.3305805, -0.1326121, 0.3746])
        # EE_POS_TGT = np.asmatrix([0.3305805, -0.1326121, 0.4868]) # center of the H
        ...
        # TREE_PATH = '/home/erle/ros_scara/src/scara_e1/scara_e1_description/urdf/scara_e1_3joints.urdf'
    ...
    def test_ppo1(self,env, num_timesteps, seed, save_model_with_prefix, restore_model_from_file):
        ...
        tf.train.Saver().restore(sess, '/home/erle/RISTO_NN/1000000_nsec/models/3dof_ppo1_test_H_afterIter_487.model') # for the H

        # file_endeffector = open('end_effector_10000000_nsec_ppo1_3dof_2.csv', 'w')

        env.start_steps()
        while rclpy.ok():
            action = pi.act(False, obs)[0]
            obs, reward, done, ee_point_temp = env.step(action)
            # file_endeffector.write(str(reward) + ", " + str(ee_point_temp[0]) + ", " + str(ee_point_temp[1]) + ", " + str(ee_point_temp[2]) + "\n")
            # if env.reward > 0.999:
            #     break;
