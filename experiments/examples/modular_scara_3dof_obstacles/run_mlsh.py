...
import mlsh_code.rollouts_robotics_mult_obst as rollouts
...
def start(callback, workerseed, rank, comm):
    env = gym.make('GazeboModularScaraStaticObstacle3DOF-v1') #Env of static obstacle with single target (center of R) and extended action_space and observation_space
    #env = gym.make('GazeboModularScaraStaticObstacle3DOF-v0') #Env of static obstacle with two fixed targets (H and O)
    ...
    env.realgoal = [0.3239543, 0.0083323, 0.3746] #center of R
...
def callback(it):
    ...
    if it == 0:
        print("CALLBACK")
        fname = '/media/erle/RL/networks/3DOF_static_obstacle_single_target/00006'
        subvars = []
